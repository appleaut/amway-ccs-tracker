...
/// Type of an interaction logged against a contact (activity history).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl ActivityKind {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ActivityKind::Demo => "Demo",
            ActivityKind::Promotion => "Promotion",
    ...
    pub fn label_th(self) -> &'static str {
        match self {
            ActivityKind::Demo => "สาธิตสินค้า",
            ActivityKind::Promotion => "บอกโปรโมชั่น",
            ActivityKind::Plan => "พูดแผนธุรกิจ",
            ActivityKind::FollowUp => "ติดตามผล",
            ActivityKind::Meeting => "นัดพบ / พูดคุย",
            ActivityKind::Other => "อื่นๆ",
        }
    }

    pub fn from_db(s: &str) -> ActivityKind {
        match s {
            "Demo" => ActivityKind::Demo,
            "Promotion" => ActivityKind::Promotion,
            "Plan" => ActivityKind::Plan,
            "FollowUp" => ActivityKind::FollowUp,
